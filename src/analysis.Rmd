...
Generate separate datasets for analysis, where we remove all NA's:

no_ias_records:

```{r}
no_ias_records_data <- 
  input_data %>% filter(!is.na(no_ias_records))
```
...
```{r}
number_of_publications_data <- 
  input_data %>% filter(!is.na(number_of_publications))
```
...
clm_records <- 
  clm(
    no_ias_records ~ design + duration + sm_yn + training_yn + map + feedback,
    data = no_ias_records_data)
```
...
```{r}
number_of_participants_data <- 
  input_data %>% filter(!is.na(number_of_participants))
```

Define `number_of_participants` as an ordered factor:

```{r}
number_of_participants_data $ number_of_participants <- factor(number_of_participants_data $ number_of_participants, ordered = TRUE)
```

Re-order `number_of_participants`:

```{r}
number_of_participants_data $ number_of_participants <- ordered (number_of_participants_data $ number_of_participants, levels = c("1 - 50", "51 - 100", 
...
```{r}
levels(number_of_participants_data$number_of_participants)
```
...
clm_participants <- 
  clm(
    number_of_participants ~ duration + design + sm_yn + training_yn + map + feedback,
    data = number_of_participants_data)
```
...
```{r}
number_of_publications_data $ number_of_publications <- factor(number_of_publications_data $ number_of_publications, ordered = TRUE)
```

Re-order `number_of_participants`:

```{r}
number_of_publications_data $ number_of_publications <- ordered (number_of_publications_data $ number_of_publications, levels = c("0", "1 to 5", "6 to 10", "11 to 20", "21 - 50", "> 100"))
```

Verify order:

```{r}
str(number_of_publications_data$number_of_publications)
```
...
clm_publications <- 
  clm(
    number_of_publications ~ duration + design + sm_yn + training_yn + map  + feedback,
    data = number_of_publications_data)
```
