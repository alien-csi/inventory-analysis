...
```{r}
input_data <- read_csv(file = here::here("data", "processed_data_2022.csv")) 
```
...
table(input_data$taxa)
table(input_data$environment)
table(input_data$audience)
```
...
```{r}
table(df_aud_tax$input_data.audience, df_aud_tax$input_data.taxa)
```
...
```{r}
table(df_aud_env$input_data.audience, df_aud_env$input_data.environment)
```
...
```{r}
table(df_reg_aud$input_data.region, df_reg_aud$input_data.audience)
```
...
```{r}
table(df_reg_tax$input_data.region, df_reg_tax$input_data.taxa)
```
...
```{r}
table(df_reg_env$input_data.region, df_reg_env$input_data.environment)
```
...
```{r}
chisq.test(df_aud_tax$input_data.audience, df_aud_tax$input_data.taxa)
```
...
```{r}
chisq.test(df_aud_env$input_data.audience, df_aud_env$input_data.environment)
```
...
```{r}
chisq.test(df_reg_aud$input_data.region, df_reg_aud$input_data.audience)
```
...
```{r}
chisq.test(df_reg_tax$input_data.region, df_reg_tax$input_data.taxa)
```
...
clm_participants <- 
  clm(
    number_of_participants ~ duration + design + sm_yn + skills + guidelines + training_yn + map + active_informing + feedback + support,
    data = input_data)
```
...
clm_publications <- 
  clm(
    number_of_publications ~ duration + design + sm_yn + skills + guidelines + training_yn + map + active_informing + feedback + support,
    data = input_data)
```
